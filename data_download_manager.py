...
from tradestation_data_collector import TradestationDataCollector
...
class DataDownloadWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # 创建Tradestation数据收集器
            self.collector = TradestationDataCollector(
            ...
            # 开始收集数据 - 收集多个时间周期
            self.collector.start_collection(['1m', '5m', '15m', '30m', '1h'])
...
class DataDownloadManager(QMainWindow):
    ...
    def __init__(self):
        ...
        # 默认期货品种
        self.default_symbols = ["ES", "NQZ25"]  # 标普500期货, 纳斯达克100期货
    ...
    def init_ui(self):
        """初始化用户界面"""
        self.setWindowTitle("数据下载管理器 - Tradestation期货")
        ...
        # 添加默认期货按钮
        self.add_default_btn = QPushButton("添加默认期货")
        self.add_default_btn.clicked.connect(self.add_default_symbols)
        symbol_management_layout.addWidget(self.add_default_btn, 0, 3)
        
        # 合约列表
        symbol_management_layout.addWidget(QLabel("下载合约列表:"), 1, 0)
        self.symbol_list = QListWidget()
        self.symbol_list.setMaximumHeight(150)
        symbol_management_layout.addWidget(self.symbol_list, 1, 1, 1, 3)
    ...
    def load_default_symbols(self):
        ...
        self.log_message(f"已加载默认期货品种: {', '.join(self.default_symbols)}")
    ...
    def add_default_symbols(self):
        ...
        if added_count > 0:
            self.log_message(f"已添加 {added_count} 个默认期货品种")
        else:
            QMessageBox.information(self, "提示", "所有默认期货品种已存在于列表中")
    ...
    def add_symbol_to_list(self, symbol):
        ...
        elif symbol in self.default_symbols:
            item.setBackground(QtGui.QColor(255, 255, 200))  # 黄色背景表示默认期货
            item.setToolTip("默认期货品种")
    ...
    def start_download(self):
        ...
        for symbol in symbols:
            if symbol not in self.download_workers:
                ...
                worker = DataDownloadWorker(symbol, db_path)
    ...
    def load_config(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                # 加载默认期货品种
                default_symbols = config.get("default_symbols", self.default_symbols)
...
def main():
    ...
    # 设置应用程序信息
    app.setApplicationName("Tradestation数据下载管理器")
