...
class StockAnalysisGUI:
    def __init__(self, root):
        ...
        self.download_progress_file = "download_progress.json"
    ...
    def create_widgets(self):
        ...
        # 下载全部A股股票数据按钮
        self.download_all_button = ttk.Button(left_frame, text="下载全部A股", command=self.download_all_stocks)
        self.download_all_button.grid(row=0, column=3, padx=5, pady=5)
        
        # 暂停/继续按钮
        self.pause_button = ttk.Button(left_frame, text="暂停", command=self.pause_download, state="disabled")
        ...
        self.progress_bar.grid(row=2, column=0, columnspan=5, sticky=(tk.W, tk.E), pady=2)
        ...
        self.start_date = DateEntry(left_frame, width=15, background='darkblue', foreground='white', 
                                   borderwidth=2, date_pattern='yyyy-mm-dd', locale='zh_CN')
        ...
        # 分析按钮
        ttk.Button(left_frame, text="分析K线数据", command=self.analyze_kline_data).grid(row=5, column=0, columnspan=5, pady=10)
        ...
        ttk.Button(left_frame, text="分析成交量数据", command=self.analyze_volume_data).grid(row=11, column=0, columnspan=5, pady=10)
        ...
        # 输出框布局 - 与a2-a6对齐
        ttk.Entry(left_frame, textvariable=self.b1_var, width=6, state='readonly').grid(row=16, column=3, padx=2, pady=2)
        ttk.Entry(left_frame, textvariable=self.b2_var, width=6, state='readonly').grid(row=16, column=4, padx=2, pady=2)
        ttk.Entry(left_frame, textvariable=self.b3_var, width=6, state='readonly').grid(row=16, column=5, padx=2, pady=2)
        ttk.Entry(left_frame, textvariable=self.b4_var, width=6, state='readonly').grid(row=16, column=6, padx=2, pady=2)
        ttk.Entry(left_frame, textvariable=self.b5_var, width=6, state='readonly').grid(row=16, column=7, padx=2, pady=2)
        
        # 计算按钮
        ttk.Button(left_frame, text="计算", command=self.calculate_values).grid(row=18, column=0, columnspan=8, pady=10)
        
        # 导出按钮
        ttk.Button(left_frame, text="导出分析结果", command=self.export_analysis_results).grid(row=19, column=0, columnspan=8, pady=10)
    ...
    def analyze_kline_data(self):
        ...
        if not data_exists:
            # 数据不存在，先下载数据
            self.add_log(f"股票 {stock_code} 数据不存在，开始下载...")
                        ...
                        for grandchild in child.winfo_children():
                            if isinstance(grandchild, ttk.LabelFrame):
                                for greatgrandchild in grandchild.winfo_children():
                                    if isinstance(greatgrandchild, ttk.Button) and greatgrandchild.cget('text') == "分析K线数据":
                                        analyze_button = greatgrandchild
                                        greatgrandchild.config(state="disabled")
                                        break
            
            # 在新线程中执行数据下载和分析
            def download_and_analyze_thread():
                try:
                    # 登录BaoStock
                    if not self.capture_print_output(self.data_collector.login_baostock):
                        self.root.after(0, lambda: messagebox.showerror("错误", "连接BaoStock失败"))
                        return
                    ...
                    # 下载完成后进行分析
                    self.root.after(0, lambda: self.perform_kline_analysis(stock_code, start_date, end_date))
                    
                except Exception as e:
                    error_msg = str(e)
                    self.root.after(0, lambda: messagebox.showerror("错误", f"下载数据失败: {error_msg}"))
                finally:
                    # 恢复分析按钮
                    if analyze_button:
                        self.root.after(0, lambda: analyze_button.config(state="normal"))
            
            # 启动下载和分析线程
            thread = threading.Thread(target=download_and_analyze_thread)
            thread.daemon = True
            thread.start()
            return
        
        # 数据存在，直接分析
        self.perform_kline_analysis(stock_code, start_date, end_date)

    def perform_kline_analysis(self, stock_code, start_date, end_date):
        """执行K线数据分析"""
        # 获取数据
        ...
        # 计算比例
        ratio = max_close_row['close'] / min_close_row['close']
        result_text += "=== 价格比例分析 ===\n"
    ...
    def analyze_volume_data(self):
        ...
        # 获取日线数据作为基础数据
        daily_df = self.get_stock_data(stock_code, start_date, end_date, 'daily')
        if daily_df is None or daily_df.empty:
            messagebox.showerror("错误", "未找到指定时间段的日线数据")
            return
        ...
        for period_name, period in period_list:
            if period == 'yearly':
                # 年线级别使用季线数据代替
                df = self.get_stock_data(stock_code, start_date, end_date, 'quarterly')
            else:
            ...
            volume_formatted = f"{total_volume / 10000:.2f}万"
            result_text += f"{period_name}级别周期内成交量合计：{volume_formatted}, 换手率为：{turnover:.2f}%\n"
        
        result_text += "\n" + "="*50 + "\n"
        result_text += "详细分析（基于日K线数据合成）\n"
        result_text += "="*50 + "\n\n"
        ...
        if self.daily_var.get():
            ...
            for _, row in daily_df_sorted.iterrows():
                date_str = row['date'].strftime('%Y-%m-%d')
                volume = row['volume']
                # 直接使用数据表中的turn字段作为换手率
                turnover = row['turn'] if 'turn' in row and pd.notna(row['turn']) else 0
                volume_formatted = f"{volume / 10000:.2f}万"
                result_text += f"{date_str} 成交量合计：{volume_formatted} 换手率：{turnover:.2f}%\n"
        ...
        if self.yearly_var.get():
            ...
            # 获取年线数据
            yearly_df = self.get_stock_data(stock_code, start_date, end_date, 'yearly')
            if yearly_df is not None and not yearly_df.empty:
                # 按日期排序
                yearly_df_sorted = yearly_df.sort_values('date')
                
                for _, row in yearly_df_sorted.iterrows():
                    date_str = row['date'].strftime('%Y-%m-%d')
                    volume = row['volume']
                    # 直接使用数据表中的turnover_rate字段作为换手率
                    turnover = row['turnover_rate'] if 'turnover_rate' in row and pd.notna(row['turnover_rate']) else 0
                    volume_formatted = f"{volume / 10000:.2f}万"
                    result_text += f"{date_str} 成交量合计：{volume_formatted} 换手率：{turnover:.2f}%\n"
            else:
                result_text += "年线数据为空\n"
        
        if self.quarterly_var.get():
            # 按季度分析 - 使用季线数据表中的实际数据
            result_text += "\n【按季度分析】\n"
            
            # 获取季线数据
            quarterly_df = self.get_stock_data(stock_code, start_date, end_date, 'quarterly')
            if quarterly_df is not None and not quarterly_df.empty:
                # 按日期排序
                quarterly_df_sorted = quarterly_df.sort_values('date')
                
                for _, row in quarterly_df_sorted.iterrows():
                    date_str = row['date'].strftime('%Y-%m-%d')
                    volume = row['volume']
                    # 直接使用数据表中的turnover_rate字段作为换手率
                    turnover = row['turnover_rate'] if 'turnover_rate' in row and pd.notna(row['turnover_rate']) else 0
        ...
        if self.monthly_var.get():
            ...
            # 获取月线数据
            monthly_df = self.get_stock_data(stock_code, start_date, end_date, 'monthly')
            if monthly_df is not None and not monthly_df.empty:
                # 按日期排序
                monthly_df_sorted = monthly_df.sort_values('date')
                
                for _, row in monthly_df_sorted.iterrows():
                    date_str = row['date'].strftime('%Y-%m-%d')
                    volume = row['volume']
                    # 直接使用数据表中的turn字段作为换手率
                    turnover = row['turn'] if 'turn' in row and pd.notna(row['turn']) else 0
        ...
        if self.weekly_var.get():
            ...
            # 获取周线数据
            weekly_df = self.get_stock_data(stock_code, start_date, end_date, 'weekly')
            if weekly_df is not None and not weekly_df.empty:
                # 按日期排序
                weekly_df_sorted = weekly_df.sort_values('date')
                
                for _, row in weekly_df_sorted.iterrows():
                    date_str = row['date'].strftime('%Y-%m-%d')
                    volume = row['volume']
                    # 直接使用数据表中的turn字段作为换手率
                    turnover = row['turn'] if 'turn' in row and pd.notna(row['turn']) else 0
    ...
    def download_all_stocks(self):
        ...
        try:
            ...
            # 更新状态显示
            self.status_label.config(text="状态: 正在获取A股股票列表...", foreground="orange")
            ...
            def download_thread():
                try:
                    self.root.after(0, lambda: self.update_progress(5, "正在连接BaoStock..."))
                    
                    # 登录BaoStock
                    if not self.capture_print_output(self.data_collector.login_baostock):
                        self.root.after(0, lambda: self.status_label.config(text="状态: 连接失败", foreground="red"))
                        self.root.after(0, lambda: self.download_all_button.config(state="normal"))
                        self.root.after(0, lambda: self.update_progress(0, "连接失败"))
                        return
                    
                    self.root.after(0, lambda: self.update_progress(10, "连接成功，正在获取A股股票列表..."))
                    ...
                    try:
                        # 检查是否有断点续传
                        progress_data = self.load_download_progress()
                        if progress_data and messagebox.askyesno("断点续传", "检测到上次下载进度，是否继续？"):
                            stock_list = progress_data['stock_list']
                            start_index = progress_data['current_index']
                            self.add_log(f"从断点继续下载，从第 {start_index + 1} 只股票开始")
                        else:
                            # 读取CSV文件中的股票列表
                            csv_file_path = 'stock_list_20241219.csv'
                            if not os.path.exists(csv_file_path):
                                self.root.after(0, lambda: self.status_label.config(text="状态: 股票列表文件不存在", foreground="red"))
                                self.root.after(0, lambda: self.download_all_button.config(state="normal"))
                                self.root.after(0, lambda: self.pause_button.config(state="disabled"))
                                self.root.after(0, lambda: self.update_progress(0, "股票列表文件不存在"))
                                self.root.after(0, lambda: messagebox.showerror("错误", f"股票列表文件不存在: {csv_file_path}"))
                                return
                            ...
                            for encoding in encodings:
                                try:
                                    stock_df = pd.read_csv(csv_file_path, encoding=encoding)
                                    self.add_log(f"成功使用 {encoding} 编码读取CSV文件")
                                    break
                                except UnicodeDecodeError:
                                    continue
                                except Exception as e:
                                    self.add_log(f"使用 {encoding} 编码读取失败: {str(e)}")
                                    continue
                            
                            if stock_df is None:
                                raise Exception("无法读取CSV文件，尝试了多种编码格式")
                            stock_list = []
                            
                            # 处理CSV数据，尝试不同的列名组合
                            self.add_log(f"CSV文件列名: {list(stock_df.columns)}")
                            ...
                            # 可能的列名组合
                            possible_code_columns = ['code', '股票代码', '代码', 'symbol', 'stock_code']
                            possible_name_columns = ['name', '股票名称', '名称', 'stock_name', '股票名']
                            ...
                            if code_column is None or name_column is None:
                                raise Exception(f"CSV文件中未找到股票代码或名称列。可用列: {list(stock_df.columns)}")
                            
                            self.add_log(f"使用列: {code_column} (代码), {name_column} (名称)")
                            
                            for _, row in stock_df.iterrows():
                                stock_code = str(row.get(code_column, '')).strip()
                            ...
                            if not stock_list:
                                self.root.after(0, lambda: self.status_label.config(text="状态: CSV文件中没有有效的股票数据", foreground="red"))
                                self.root.after(0, lambda: self.download_all_button.config(state="normal"))
                                self.root.after(0, lambda: self.pause_button.config(state="disabled"))
                                self.root.after(0, lambda: self.update_progress(0, "CSV文件中没有有效的股票数据"))
                                self.root.after(0, lambda: messagebox.showerror("错误", "CSV文件中没有有效的股票数据"))
                                return
                            
                            start_index = 0
                        
                        self.root.after(0, lambda: self.update_progress(15, f"从CSV文件读取到 {len(stock_list)} 只A股股票，开始下载数据..."))
                        
                        # 计算每个股票的进度
                        progress_per_stock = 85.0 / (len(stock_list) - start_index)
                        current_progress = 15
                        ...
                        for i in range(start_index, len(stock_list)):
                            ...
                            try:
                                ...
                                self.root.after(0, lambda code=stock_code, name=stock_name, idx=i: 
                                    self.update_progress(current_progress + (idx - start_index) * progress_per_stock, 
                                                       f"正在下载 {code} ({name}) 的数据... ({idx + 1}/{len(stock_list)})"))
                                
                                # 使用现有的查询功能下载股票数据
                                self.capture_print_output(self.data_collector.collect_complete_stock_data, stock_code, stock_name)
                                
                                # 保存进度
                                self.save_download_progress(stock_list, i + 1)
                                
                            except Exception as e:
                                error_msg = str(e)
                                self.root.after(0, lambda code=stock_code, error=error_msg: 
                                    self.add_log(f"下载 {code} 时出错: {error}"))
                                continue
                        
                        # 完成
                        if self.download_stopped:
                            self.root.after(0, lambda: self.status_label.config(text="状态: 下载已停止", foreground="orange"))
                            self.root.after(0, lambda: self.update_progress(0, "下载已停止"))
                            self.root.after(0, lambda: messagebox.showinfo("停止", "下载已停止"))
                        else:
                            self.root.after(0, lambda: self.status_label.config(text="状态: 全部A股数据下载完成", foreground="green"))
                            self.root.after(0, lambda: self.update_progress(100, "全部A股数据下载完成"))
                            self.root.after(0, lambda: messagebox.showinfo("成功", f"全部A股数据下载完成，共下载 {len(stock_list)} 只股票"))
                            # 清除进度文件
                            self.clear_download_progress()
                        
                    except Exception as e:
                        error_msg = str(e)
                        self.root.after(0, lambda: self.status_label.config(text="状态: 读取CSV文件失败", foreground="red"))
                        self.root.after(0, lambda: self.download_all_button.config(state="normal"))
                        self.root.after(0, lambda: self.pause_button.config(state="disabled"))
                        self.root.after(0, lambda: self.update_progress(0, f"读取CSV文件失败: {error_msg}"))
                        self.root.after(0, lambda: messagebox.showerror("错误", f"读取CSV文件失败: {error_msg}"))
                        
                except Exception as e:
                    error_msg = str(e)
                    self.root.after(0, lambda: self.status_label.config(text=f"状态: 错误 - {error_msg}", foreground="red"))
                    self.root.after(0, lambda: self.update_progress(0, f"错误: {error_msg}"))
                    self.root.after(0, lambda: messagebox.showerror("错误", f"下载全部A股数据失败: {error_msg}"))
                finally:
                    self.root.after(0, lambda: self.download_all_button.config(state="normal"))
                    self.root.after(0, lambda: self.pause_button.config(state="disabled"))
            ...
            error_msg = str(e)
            messagebox.showerror("错误", f"启动下载任务失败: {error_msg}")
    ...
    def pause_download(self):
        ...
        # 长按停止功能（通过右键菜单实现）
        def show_stop_menu(event):
            menu = tk.Menu(self.root, tearoff=0)
            menu.add_command(label="停止下载", command=self.stop_download)
            menu.post(event.x_root, event.y_root)
        
        # 绑定右键菜单
        self.pause_button.bind("<Button-3>", show_stop_menu)
    ...
    def save_download_progress(self, stock_list, current_index):
        """保存下载进度"""
        import json
        progress_data = {
        ...
        try:
            with open(self.download_progress_file, 'w', encoding='utf-8') as f:
                json.dump(progress_data, f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.add_log(f"保存进度失败: {str(e)}")

    def load_download_progress(self):
        """加载下载进度"""
        import json
        try:
            if os.path.exists(self.download_progress_file):
                with open(self.download_progress_file, 'r', encoding='utf-8') as f:
                    progress_data = json.load(f)
                return progress_data
        except Exception as e:
            self.add_log(f"加载进度失败: {str(e)}")
        return None
    ...
    def clear_download_progress(self):
        ...
        try:
            if os.path.exists(self.download_progress_file):
                os.remove(self.download_progress_file)
                self.add_log("下载进度文件已清除")
        except Exception as e:
            self.add_log(f"清除进度文件失败: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print("警告: 无法设置中文语言环境，日期选择器可能显示英文")
